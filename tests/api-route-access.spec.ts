...
test.describe("api route access controls (static)", () => {
  ...
  test("repo no longer includes owner debug inspection endpoints", () => {
    expect(() => readFileSync("src/app/api/admin/debug/route.ts", "utf8")).toThrow();
    expect(() => readFileSync("src/app/api/admin/dbinfo/route.ts", "utf8")).toThrow();
  });
});
