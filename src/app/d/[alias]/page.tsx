...
export default async function AliasDocPage({
  ...
  // Normalize just in case (emails/clients can accidentally uppercase)
  const alias = decodeURIComponent(params.alias || "").trim().toLowerCase();

  if (!alias) notFound();

  // IMPORTANT:
  // Use public.doc_aliases (explicit schema) to avoid search_path surprises.
