...
function intentLinks(intent: AccessIntent) {
  const callback = intent === "admin" ? "/auth/continue-admin" : "/auth/continue-viewer";
  const callbackQs = encodeURIComponent(callback);
  return {
    google: `/api/auth/signin/google?callbackUrl=${callbackQs}`,
    enterprise: `/api/auth/signin/enterprise-sso?callbackUrl=${callbackQs}`,
    email: `/signin/manual?intent=${intent}`,
  };
}
...
export default function SignInPage() {
  const [intent, setIntent] = useState<AccessIntent>("admin");

  const activeCard = useMemo(
    () => ACCESS_CARDS.find((card) => card.key === intent) ?? ACCESS_CARDS[0],
    [intent]
  );
  const links = useMemo(() => intentLinks(intent), [intent]);
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        setIntent(card.key);
                        try {
                          window.localStorage.setItem("doclinks_signin_intent", card.key);
                        } catch {
            ...
            <div className="rounded-2xl border border-white/12 bg-white/[0.03] p-5 sm:p-6">
              ...
              <div className="mt-5 space-y-3">
                {googleConfigured ? (
                  <a
                    href={links.google}
                    className="btn-base block rounded-xl border border-cyan-300/35 bg-cyan-300 px-4 py-3 text-sm font-semibold text-[#07131f] shadow-[0_14px_34px_rgba(34,211,238,0.2)] hover:bg-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70"
                  >
                    Continue with Google
                  </a>
                ) : (
                  <div className="rounded-xl border border-white/12 bg-white/[0.04] px-4 py-3 text-sm text-white/60">
                ...
                {enterpriseConfigured ? (
                  <a
                    ...
                    className="btn-base block rounded-xl border border-white/14 bg-white/[0.05] px-4 py-3 text-sm text-white/86 hover:border-white/26 hover:bg-white/[0.1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70"
                  >
                    Continue with Enterprise SSO
                  </a>
                ) : (
                  <div className="rounded-xl border border-white/12 bg-white/[0.03] px-4 py-3">
                    <div className="text-sm text-white/66">Continue with Enterprise SSO</div>
                    <div className="mt-1 text-xs text-white/45">Available for organizations with SSO enabled.</div>
                  </div>
                )}
