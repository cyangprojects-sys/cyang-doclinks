...
export type ShareMeta =
    | {
        ...
        token: string;
        /**
         * Email-bound enforcement:
         * - Prefer allowed_email
         * - Fallback to legacy to_email
         */
        allowedEmail: string | null;
        toEmail: string | null;
        /**
         * Download control. Default false.
         */
        allowDownload: boolean;
        createdAt: string;
...
export async function resolveShareMeta(tokenInput: string): Promise<ShareMeta> {
    ...
    // Prefer doc_shares
    try {
        // Attempt with new columns (allowed_email, allow_download)
        try {
            const rows = (await sql`
        select
          token::text as token,
      ...
      `) as unknown as Array<{
            ...
            if (rows?.length) {
                ...
                return {
                    ...
                    allowedEmail: r.allowed_email ?? r.to_email ?? null,
...
> {
    ...
    try {
        ...
        returning s.doc_id::text as doc_id, s.password_hash, s.allowed_email, s.to_email, s.allow_download
      `) as unknown as Array<{
                doc_id: string;
                password_hash: string | null;
                allowed_email: string | null;
            ...
            if (rows?.length) {
                return {
                    ...
                    table: "doc_shares",
                    allowedEmail: rows[0].allowed_email ?? rows[0].to_email ?? null,
                    allowDownload: Boolean(rows[0].allow_download),
                };
            }
        } catch {
            const rows = (await sql`
        ...
        where s.token = ${token}
          ...
          and (s.expires_at is null or s.expires_at > now())
          and (s.max_views is null or s.max_views = 0 or s.view_count < s.max_views)
        returning s.doc_id::text as doc_id, s.password_hash, s.to_email
      `) as unknown as Array<{
                doc_id: string;
                password_hash: string | null;
                to_email: string | null;
            ...
            if (rows?.length) {
                return {
                    ...
                    passwordHash: rows[0].password_hash ?? null,
                    table: "doc_shares",
                    allowedEmail: rows[0].to_email ?? null,
                    allowDownload: false,
                };
            }
        }
    ...
    try {
        ...
        returning st.doc_id::text as doc_id, st.password_hash, st.allowed_email, st.to_email, st.allow_download
      `) as unknown as Array<{
                doc_id: string;
                password_hash: string | null;
                allowed_email: string | null;
        ...
        where (st.token::text = ${token} or st.token = ${token})
          ...
          and (st.expires_at is null or st.expires_at > now())
          and (st.max_views is null or st.max_views = 0 or st.views_count < st.max_views)
        returning st.doc_id::text as doc_id, st.password_hash, st.to_email
      `) as unknown as Array<{
                doc_id: string;
                password_hash: string | null;
                to_email: string | null;
